    ...
    return async (data, fu)=> {
        try{
            ...
            if( data.notifyType === 0 ) {
                result = await user_db.findById(socket.account._id, {'notify': {$slice: [1 + (data.currentPage - 1)*10, data.currentPage*10]}});
            } else {

                result = await user_db.findOne({'_id': socket.account._id, 'notify.ntype': data.notifyType}, {'notify': {$slice: [1 + (data.currentPage - 1)*10, data.currentPage*10]}});
            }
            if( !result ) return fu([]); 
            fu( result.notify );
        } catch (err) {
